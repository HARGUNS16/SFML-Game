...
// {
//     this->window = new RenderWindow(this->v, "Game Window!!", Style::Titlebar | Style::Close | Style::Resize);
...
//     // bSprite.setScale(Vector2f(0.2f, 0.2f));
// }
...
//     sprite.setPosition(Vector2f(0.f, 600.f));
// }
...
//             // if(this->life == 2)
//             // {
//             //     texture2.loadFromFile("RedDog1.png");
//             //     this->sprite.setTexture(texture2);
//             // }
//             // else if(this->life == 1)
//             // {
...
//             // else
//             // {
//             //     texture2.loadFromFile("RedDog3.png");
//             //     this->sprite.setTexture(texture2);
//             // }
//         }
//         else
...
//             {
//                 this->sprite.setPosition(Vector2f(this->sprite.getGlobalBounds().width, 600.f));
//             }
//         }
...
//             {
//                 this->sprite.setPosition(Vector2f(this->window->getSize().x - this->sprite.getGlobalBounds().width , 600.f));
//             }
//         }
...
//             // this->moveSpriteUp = sqrt(2*g*100);
//         }
...
//         }
//         if(this->sprite.getPosition().y <= 510.f || !(this->isPlayerGoingUp))
//         {
//             this->isPlayerGoingUp = false;
//             this->sprite.setPosition(this->sprite.getPosition().x, this->sprite.getPosition().y + 4);
//         }
//         if(this->sprite.getPosition().y >= 600.f) 
//             this->isPlayerJumping = false;
//     }
// }
...
void Game::initSprite()
{
    ...
    Restart2.setPosition(Vector2f(window->getSize().x/2 - Restart2.getGlobalBounds().width/2, window->getSize().y/2));
    Restart.setTexture(reTexture);
    Restart.setPosition(Vector2f(window->getSize().x/2 - Restart.getGlobalBounds().width/2, window->getSize().y/2));
    ...
    start.setPosition(Vector2f(window->getSize().x/2 - 6*start.getGlobalBounds().width/5, window->getPosition().y + 400));
    start2.setPosition(Vector2f(window->getSize().x/2 - 6*start2.getGlobalBounds().width/5, window->getPosition().y + 400));
    exit.setPosition(Vector2f(window->getSize().x/2 + 2*exit.getGlobalBounds().width/5, window->getPosition().y + 398));
    exit2.setPosition(Vector2f(window->getSize().x/2 + 2*exit2.getGlobalBounds().width/5, window->getPosition().y + 398));
    heading.setPosition(Vector2f(window->getSize().x/2 - heading.getGlobalBounds().width/2, window->getPosition().y + 60));
}
...
void Game::initFonts()
{
    ...
    Over.setPosition(Vector2f(window->getSize().x/2 - Over.getGlobalBounds().width/2, window->getSize().y/2 - 3*Over.getGlobalBounds().height));
...
void Game::addNewBall()
{
    ...
    this->circle.setFillColor(Color(255, 0, 0, 255));
...
void Game::updateBallPosition()
{
    ...
    //Adding new ball if specified time is over and maximum ball limit is not exceeded
    if ((m == 0) && (this->circleVector.size() < this->maxBalls) && (this->po <= 8))
    {
        this->addNewBall();
    }
...
void Game::updateBallPosition2()
{
    ...
    if ((circles.getPosition().x >= window->getSize().x - 19) || (circles.getPosition().x <= 9))
    {
        vx = -vx;
    }
...
void Game::update()
{
    ...
    {
        this->updateSprite();
        if ((this->po <= 8) || (this->circleVector.size() > 0))
        {
            this->updateBallPosition();
        }
        else
        {
            if (i1 == 0)    this->initVariables2();
